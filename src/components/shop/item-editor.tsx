...
export const ItemEditor=(props:ItemEditorProps)=>{
  ...
  const {item:{id},index,onToggle,onUpdate,onCancel}=props;
  ...
  <div className="border border-danger">
    <div className="row bg-danger mx-0 justify-content-between align-items-center">
      ...
      <button type="button" className="col btn btn-sm btn-primary rounded-0" title="Enregistrer" onClick={onUpdate}>
        <span className="material-icons">save</span>
      </button>
      <button type="button" className="col btn btn-sm btn-warning rounded-0" title="Annuler" onClick={onCancel}>
        <span className="material-icons">close</span>
      </button>
    </div>      
    <div id={'key-'+id} className={`${collapse} row justify-content-between align-items-center mx-0 g-1 p-1 bg-white`}>
      <div className="col-3">
      ...
      <table className="table table-sm  table-dark caption-top align-middle">
        ...
        {props.item.rates.map((r,j)=>
          ...
          <tr>
            <td colSpan={3}>
              <button type="button" className="btn btn-sm btn-primary" onClick={props.onAddRate}>
                Ajouter un tarif
              </button>
            </td>
          </tr>
